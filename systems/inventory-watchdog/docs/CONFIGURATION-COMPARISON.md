...
*Three representative deployment patterns*

> Company names are fictional composites and all figures are illustrative of representative deployments — not audited results attributable to any specific company.
...
**Solution:** Track by product line prefix + specific SKUs for the flagship collection  
...
```json
{
  ...
  "product_lines": [
    {"name": "Line A (Sport)", "prefix": "SKU-1", "track_all_variants": true, "priority": "critical"},
    {"name": "Line C (Classic)", "prefix": "SKU-3", "track_all_variants": false, "specific_skus": ["SKU-3030", "SKU-3035"], "priority": "high"}
  ],
...
```json
{
  "late_orders": {
    ...
    "exclusions": {"tags": ["rx-order", "lab", "custom lenses"]}
  }
}
```
...
**Channels:** Example allocation targets — DTC 40%, Amazon US 35%, Amazon EU 10%, Wholesale 15%  
...
**Suppliers:**
- Supplier A (China) — 21 days, MOQ 50
- Supplier B (Taiwan) — 30 days, MOQ 100
- Supplier C (USA, optical lab) — 14 days, MOQ 25
...
```json
{
  "reorder_process": {
    ...
    "suppliers": [
      {"id": "supplier-a", "name": "Supplier A (China)", "lead_time_days": 21, "minimum_order_quantity": 50}
    ]
  }
}
```
...
```json
{
  "alerts": {
    "recipients": [
      {"name": "COO", "delivery_channel": "telegram", "severity_filter": ["critical", "warning"]},
