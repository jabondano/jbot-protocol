...
**Example (reference deployment):** NetSuite (Full ERP, SuiteQL API integrated)
...
**Example (reference deployment):** Admin, OAuth 1.0 API access, real-time SuiteQL queries
...
**Example (reference deployment):** ~900 active SKUs, filtered by product line (4 active lines)
...
|---------------|----------------------------------|---------|-------------------|
| Example: Main Warehouse | Warehouse | USA | Yes |
| Example: Amazon FBA US | Amazon FBA | USA | Yes |
| Example: Netherlands 3PL | 3PL | Netherlands | Yes |

**Example (reference deployment):** 15 locations (primary US warehouse, 2 Amazon FBA locations, 12 international 3PL locations)
...
**Example (reference deployment):** DTC website (Shopify), Amazon FBA (US + DE), Wholesale (B2B portal)
...
**Example (reference deployment):** Flexible targets — DTC and wholesale can borrow from each other, Amazon FBA is ring-fenced (requires manual rebalancing)
...
**Example (reference deployment top 3):**
...
**Example (reference deployment, before):** 8 stockouts in 90 days, avg $2,700 lost revenue per stockout = $21,600 lost
...
**Example (reference deployment, before):** 2-3 times per day, 20-30 minutes per check, ops manager = 15-20 hrs/week
...
**Example (reference deployment):** Days of supply — Warning <14 days, Critical <7 days, using 30-day velocity
...
**Example (reference deployment):** No overstock alerts (warehouse space not a constraint, prefer safety stock)
...
**Example (reference deployment):** Yes, track open POs and expected arrival dates — don't alert if PO arriving <7 days
...
**Example (reference deployment):** Yes — Standard products >7 days = monitor, >14 days = attention, >30 days = critical. Rx/custom orders excluded (expected long cycle).
...
**Example (reference deployment):** Exclude Rx orders (tagged "rx-order", "lab", "custom lenses"), made-to-order
...
**Example (reference deployment):** Semi-automated — Watchdog alerts, ops manager reviews and places PO in NetSuite
...
|---------------|------------------|-------------------|---------------|-------------------|
| Example: Supplier A | 21 | 50 units | SKU-1xxx, SKU-2xxx | Yes |
| Example: Supplier B | 30 | 100 units | SKU-5xxx | No (requires approval) |

**Example (reference deployment):** 4 primary suppliers (Suppliers A-D, spanning overseas manufacturers and a domestic optical lab), 14-45 day lead times
...
**Example (reference deployment):** Yes — Generate PO recommendation with SKU, quantity, supplier, lead time, expected delivery. Ops manager reviews before placing in NetSuite.
...
|------|------|-------------|------------------|--------------|
| Example: COO | COO | Critical stockouts, high-value late orders | Telegram | (chat ID) |
...
- COO: Critical stockouts, aged POs, high-value late orders → Telegram
...
**Example (reference deployment):** Telegram (primary), Discord (backup for daily digests)
...
**Example (reference deployment):** Every 4 hours (10am, 2pm, 6pm, 10pm ET) for inventory watchdog, daily 9:30am for late orders
...
**Example (reference deployment):** No quiet hours (COO wants critical alerts 24/7, warehouse email digests sent during business hours)
...
**Example (reference deployment targets):**
